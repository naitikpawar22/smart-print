...
const Mobile_NoEl = document.querySelector('.form-container .form-row input[name="Mobile_No"]');
...
let nameValid = false;
let Mobile_NoValid = false;

const Mobile_NoTl = createMobile_NoTl();
...
Mobile_NoEl.addEventListener('input', () => {
    const Mobile_NoRegex = /^\+?[1-9]\d{1,14}$/;
    Mobile_NoValid = Mobile_NoRegex.test(Mobile_NoEl.value);
    if (Mobile_NoValid) {
        Mobile_NoTl.play();
        Mobile_NoEl.classList.add("valid");
    } else {
        Mobile_NoTl.reverse();
        Mobile_NoEl.classList.remove("valid");
    }
})

submitBtn.addEventListener('click', () => {
    if (Mobile_NoValid && checkboxEl.checked && nameValid && sprayRepeatCounter > 1) {
        gsap.to("svg > *", {
            duration: .1,
            opacity: 0,
            stagger: {
